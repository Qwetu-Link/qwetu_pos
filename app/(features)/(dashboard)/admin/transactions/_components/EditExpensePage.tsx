...
import { ArrowLeft, Save, Trash2, WalletCards, Plus } from "lucide-react";
...
export default function EditExpensePage({ expenseId }: { expenseId: string }) {
  ...
  const { append, fields, remove, replace } = useFieldArray({ control, name: "items" });
  ...
  useEffect(() => {
    ...
    const nextItems = expense.items.length
    ...
    reset({
      ...
      amount: category === "inventory_purchase" ? 0 : expense.amount,
      method: expense.method as ExpensePageValues["method"],
  ...
  async function submitExpense(values: ExpensePageValues) {
    ...
    router.push(`/admin/transactions/expenses/${expenseId}`);
  }
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-5xl space-y-5 px-4 py-6 sm:px-6 lg:px-8">
        <div>
          <Link
            href={expense ? `/admin/transactions/expenses/${expense.id}` : "/admin/transactions/expenses"}
          ...
          <p className="mt-1 text-slate-500">
            Update vendor details, amount, items, and approval state.
          </p>
        </div>
