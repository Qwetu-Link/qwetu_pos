...
export default function Sidebar({
  ...
  const handleNavClick = (href: string) => {
    onTabChange?.(href);
    setIsOpen(false); // Close mobile drawer on choice
  };
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 w-[280px] bg-white border-r border-slate-100 flex flex-col justify-between shadow-xl shadow-slate-900/5 transition-transform duration-300 ease-in-out md:translate-x-0 ${
      ...
      >
            ...
            {filteredNavItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-1.5 rounded-xl text-md font-medium transition-all group duration-200 ${
                    isActive
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => alert("Demo logout hook initiated.")}
